...
class CcxtSignalExecutor:
    ...
    def _execute_signal_sync(self, payload: dict[str, Any]) -> ExecutionResult:
        ...
        if action == "UPDATE_TP_SL":
            self._logger.info("UPDATE_TP_SL action received — no order created (modify logic required)")
            return ExecutionResult(
                mode="dry-run" if self._config.execution_mode == "dry-run" else "live",
                order_id=None,
                details={"signal": payload, "note": "UPDATE_TP_SL requires modify_order — not implemented yet"},
            )
